...
class DQN(nn.Module):
    ...
    """The model outputs the predicted value 
...
def select_action(steps_done, 
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            return policy_net(state), steps_done # CHANGE
    else:
        return torch.tensor([[env.action_space.sample()]], device=device, dtype=torch.long), steps_done 


def plot_durations(episode_durations, is_ipython, show_result=False):# CHANGE
    plt.figure(1)
...
def optimize_model(memory, 
    ...
    state_action_values = policy_net(state_batch).gather(1, action_batch) # CHANGE
    ...
    with torch.no_grad():
        next_state_values[non_final_mask] = target_net(non_final_next_states).max(1)[0] # CHANGE
    # Compute the expected Q values
    expected_state_action_values = (next_state_values * gamma) + reward_batch

    # Compute Huber loss
    criterion = nn.SmoothL1Loss()
    loss = criterion(state_action_values, expected_state_action_values.unsqueeze(1)) # CHANGE?
...
def main(config):
    ...
    n_observations = len(state) # CHANGE
    print(env.action_space)
    n_actions = env.action_space.n # CHANGE

    print(f'The size of the observation space {n_observations}')
